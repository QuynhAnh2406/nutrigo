...
function Ingredients({ mode = 'ingredient' }) {
    ...
    return ( <
        div className = "ingredients-page main-content"
        style = {
            { overflowY: 'auto', paddingBottom: '80px' } } >
        <
        div className = "mb-8 relative z-30" >
        <
        PageHeader title = { mode === 'ingredient' ? 'Nguyên liệu cơ bản' : 'Thương hiệu / Đồ ăn ngoài' }
        ...
        { /* Advanced Filter Popover */ } {
            showAdvancedFilter && ( <
                ...
                className = { `px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 ${selectedCategory === 'food' ? 'bg-[#B5E361] text-[#1f3b00]' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}` } > 🍲Đồ ăn <
                /button> <
                button type = "button"
                onClick = {
                    () => setSelectedCategory('drink')
                }
                className = { `px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 ${selectedCategory === 'drink' ? 'bg-[#B5E361] text-[#1f3b00]' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}` } > 🥤Đồ uống <
                /button> <
                button type = "button"
                onClick = {
                    () => setSelectedCategory('snack')
                }
                className = { `px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 ${selectedCategory === 'snack' ? 'bg-[#B5E361] text-[#1f3b00]' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}` } > 🍪Ăn vặt <
                /button> < /
                div > <
                /div>
                ...
                { /* Row 2: Brand Text Filter (only show if mode is not 'ingredient') */ } {
                    mode !== 'ingredient' && ( <
                        ...
                        onChange = {
                            (e) => setBrandSearchQuery(e.target.value)
                        }
            ...
            filteredIngredients.length > 0 ? ( <
                div className = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in-50 duration-500" > {
                    filteredIngredients.map(ing => ( <
                        ...
                        div >
                        <
                        h4 className = "font-extrabold text-[#183000] text-base leading-snug truncate mb-3"
                        title = { ing.name } > { ing.name } <
                        /h4>
                        ...
                        <
                        Flame size = { 12 }
                        className = "fill-orange-600" / > Năng lượng <
                        /span> <
                        span className = "text-base font-black text-orange-600" > { calcVal(ing.calories_per_100g) } < small className = "text-[10px] font-bold" > kcal < /small> <
                        /span> <
                        /div>
                        ...
                        <
                        span className = {
                            (ing.type || 'ingredient') === 'brand' ? 'text-purple-600 bg-purple-50 px-2 py-0.5 rounded-lg font-extrabold' : 'text-gray-400 bg-gray-50 px-2 py-0.5 rounded-lg' } > {
                            (ing.type || 'ingredient') === 'brand' ? `🏬 ${ing.brand_name || 'Đồ ăn ngoài'}` : '🍏 Nguyên liệu' } <
                        /span> <
                        span className = "text-gray-500 bg-gray-100/60 px-2 py-0.5 rounded-lg font-extrabold" > { ing.category === 'drink' ? '🥤 Đồ uống' : ing.category === 'snack' ? '🍪 Ăn vặt' : '🍲 Đồ ăn' } <
                        /span> <
                        /div> <
        ...
        { /* Add Ingredient Modal */ } {
            isModalOpen && ( <
                ...
                <
                Sparkles className = "h-5 w-5 text-amber-500" / > Thêm { mode === 'ingredient' ? 'nguyên liệu' : 'món ăn' }
                mới <
                /h3> <
                button onClick = {
                    () => setIsModalOpen(false)
                ...
                <
                div className = "grid grid-cols-2 gap-3" > { /* Calories */ } <
                div className = "form-group flex flex-col gap-1" >
                <
                ...
                button type = "submit"
                disabled = { loading }
                className = "flex-1 py-2.5 bg-gradient-to-r from-[#B5E361] to-[#8CB33D] text-[#1f3b00] hover:scale-[1.02] active:scale-[0.98] rounded-xl font-extrabold text-sm transition-all shadow-sm" > { loading ? 'Đang lưu...' : 'Lưu lại' } <
                /button> < /
                div > <
                /form> < /
                div > <
            ...
            selectedDetailIng && ( <
                ...
                <
                span className = {
                    (selectedDetailIng.type || 'ingredient') === 'brand' ? 'text-purple-600 bg-purple-50 px-2 py-0.5 rounded-lg font-extrabold text-[9px] uppercase tracking-widest' : 'text-gray-400 bg-gray-50 px-2 py-0.5 rounded-lg font-extrabold text-[9px] uppercase tracking-widest'
                } > {
                    (selectedDetailIng.type || 'ingredient') === 'brand' ? `🏬 ${selectedDetailIng.brand_name || 'Đồ ăn ngoài'}` : '🍏 Nguyên liệu'
                } <
                /span> <
                span className = "text-gray-500 bg-gray-100/60 px-2 py-0.5 rounded-lg font-extrabold text-[9px] uppercase tracking-widest" > { selectedDetailIng.category === 'drink' ? '🥤 Đồ uống' : selectedDetailIng.category === 'snack' ? '🍪 Ăn vặt' : '🍲 Đồ ăn' } <
                /span> < /
                div > <
                h3 className = "font-extrabold text-gray-900 text-xl leading-snug" > { selectedDetailIng.name } <
                /h3> < /
                div > <
                button onClick = {
                ...
                <
                Flame size = { 18 }
                className = "fill-orange-600" / > { calcVal(selectedDetailIng.calories_per_100g, 100, selectedDetailIng) } < small className = "text-[11px] font-bold" > kcal < /small> < /
                span > <
                /div> <
                ...
                Khẩu phần <
                /span> <
                span className = "text-base font-bold text-gray-700 mt-1 block" > { selectedDetailIng.serving_unit || '100g' } <
                /span> < /
                div > <
                /div>
                ...
                <
                span className = "font-black text-[#1f3b00]" > { calcVal(selectedDetailIng.protein_per_100g, 100, selectedDetailIng) }
                g < /span> < /
                div >
                ...
                <
                span className = "font-black text-[#1f3b00]" > { calcVal(selectedDetailIng.carbs_per_100g, 100, selectedDetailIng) }
                g < /span> < /
                div >
                ...
                <
                span className = "font-black text-[#1f3b00]" > { calcVal(selectedDetailIng.fat_per_100g, 100, selectedDetailIng) }
                g < /span> < /
                div >
                ...
                <
                span className = "font-black text-[#1f3b00]" > { calcVal(selectedDetailIng.fiber_per_100g, 100, selectedDetailIng) }
                g < /span> < /
                div > <
                /div> < /
                div > <
