        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in-50 duration-500">
          {filteredIngredients.map((ing) => (
            ...
            >
              <div>
                <h4
                  className="font-extrabold text-[#183000] text-base leading-snug truncate mb-3"
                  title={ing.name}
                >
                  {ing.name}
                </h4>
                    ...
                    <span
                      className={
                        (ing.type || 'ingredient') === 'brand'
                          ? 'text-purple-600 bg-purple-50 px-2 py-0.5 rounded-lg font-extrabold'
                          : 'text-gray-400 bg-gray-50 px-2 py-0.5 rounded-lg'
                      }
                    >
                      {(ing.type || 'ingredient') === 'brand'
                        ? `🏬 ${ing.brand_name || 'Đồ ăn ngoài'}`
                        : '🍏 Nguyên liệu'}
                    </span>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[24px] p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-300 flex flex-col">
            <div className="flex justify-between items-start mb-5 border-b border-gray-50 pb-4">
              <div className="flex-1 pr-4">
                <div className="flex flex-wrap gap-1.5 mb-2">
                  <span
                    className={
                      (selectedDetailIng.type || 'ingredient') === 'brand'
                        ? 'text-purple-600 bg-purple-50 px-2 py-0.5 rounded-lg font-extrabold text-[9px] uppercase tracking-widest'
                        : 'text-gray-400 bg-gray-50 px-2 py-0.5 rounded-lg font-extrabold text-[9px] uppercase tracking-widest'
                    }
                  >
                    {(selectedDetailIng.type || 'ingredient') === 'brand'
                      ? `🏬 ${selectedDetailIng.brand_name || 'Đồ ăn ngoài'}`
                      : '🍏 Nguyên liệu'}
                  </span>
            ...
            <div className="mb-5 bg-orange-50/30 border border-orange-100/50 rounded-2xl p-4 flex justify-between items-center text-center">
              <div className="flex-1 border-r border-orange-100/50">
                ...
                <span className="text-2xl font-black text-orange-600 flex items-center justify-center gap-1">
                  <Flame size={18} className="fill-orange-600" /> {calcVal(selectedDetailIng.calories_per_100g)}
                  <small className="text-[11px] font-bold">kcal</small>
                </span>
              </div>
            ...
            <div className="space-y-3">
              <span className="block text-[10px] text-gray-400 font-black uppercase tracking-wider mb-2">
              ...
              <div className="flex justify-between items-center text-sm bg-gray-50/50 p-2.5 rounded-xl border border-gray-100/50">
                <span className="font-bold text-gray-600 flex items-center gap-2">
                  <span className="text-lg">🥩</span> Đạm (Protein)
                </span>
                <span className="font-black text-[#1f3b00]">
                  {calcVal(selectedDetailIng.protein_per_100g)} g
                </span>
              </div>
              <div className="flex justify-between items-center text-sm bg-gray-50/50 p-2.5 rounded-xl border border-gray-100/50">
                <span className="font-bold text-gray-600 flex items-center gap-2">
                  <span className="text-lg">🍞</span> Tinh bột (Carbs)
                </span>
                <span className="font-black text-[#1f3b00]">
                  {calcVal(selectedDetailIng.carbs_per_100g)} g
                </span>
              </div>
              <div className="flex justify-between items-center text-sm bg-gray-50/50 p-2.5 rounded-xl border border-gray-100/50">
                <span className="font-bold text-gray-600 flex items-center gap-2">
                  <span className="text-lg">🥑</span> Chất béo (Fat)
                </span>
                <span className="font-black text-[#1f3b00]">
                  {calcVal(selectedDetailIng.fat_per_100g)} g
                </span>
              </div>
              <div className="flex justify-between items-center text-sm bg-gray-50/50 p-2.5 rounded-xl border border-gray-100/50">
                <span className="font-bold text-gray-600 flex items-center gap-2">
                  <span className="text-lg">🥗</span> Chất xơ (Fiber)
                </span>
                <span className="font-black text-[#1f3b00]">
                  {calcVal(selectedDetailIng.fiber_per_100g)} g
                </span>
              </div>
            </div>
          </div>
